...
class Glove(object):
    ...
    def fit(self, matrix, epochs=5, no_threads=2, verbose=False):
        ...
        if self.word_vectors is None:
           self.word_vectors = ((random_state.rand(shape[0],self.no_components) - 0.5) / self.no_components)

        if self.word_biases is None:
           self.word_biases = np.zeros(shape[0], dtype=np.float64)

        if self.vectors_sum_gradients is None:
            self.vectors_sum_gradients = np.ones_like(self.word_vectors)

        if self.biases_sum_gradients is None:
    ...
    @classmethod
    def load_stanford(cls, filename, verbose=False):
        """
    ...
    def _index_by_word(self, word):
        ...
        try:
            ...
            raise Exception(f'Word not in dictionary: {word}')

    def sentence_vector(self, sentence):
        sent_embedding = np.zeros(self.no_components)
        for word in sentence:
            word_embedding = self.word_vector(word)
            sent_embedding = np.sum([sent_embedding, word_embedding], axis=0)

        sent_embedding = sent_embedding/len(sentence)
